...
def get_current_valute(to_monitor):
    ...
    valute = data['Valute']
    values = ['USD', 'EUR', 'RUB']
    # способ форматирования json в человекочитаемый формат через json.dumps
    # где выставлен параментр indent=2
    #
    # так же возможен способ через модуль стандартной библиотеки `pprint`

    # print(json.dumps(valute, indent=2))
